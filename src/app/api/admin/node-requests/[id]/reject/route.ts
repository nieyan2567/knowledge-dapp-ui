...
/**
 * 模块说明：节点申请拒绝接口，负责把指定节点申请标记为 rejected。
 */
...
import { AdminStoreConflictError, AdminStoreNotFoundError, reviewNodeRequest } from "@/server/admin/store";
 ...
 * @returns 包含拒绝后节点申请记录的 JSON 响应。
 */
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  const rateLimit = await enforceApiRateLimits(req.headers, ["admin:node-requests:reject"]);
  ...
  try {
    ...
    await captureServerException("Failed to reject node request", {
      source: "api.admin.node-requests.reject",
      severity: "error",
      request: req,
      error,
    ...
    return NextResponse.json({ error: "节点申请拒绝失败，请稍后重试" }, { status: 500 });
  }
}
